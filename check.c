...
/*
 * This file contains the code which walks two textual kabi definitions and
 * compares the two.
 */
...
static bool check_symbol_file(char *kabi_path, void *arg) {
	...
	/* If conf->kabi_dir_* contains slashes, skip them */
...
void check_symbol_defs(check_config_t *conf) {
	printf("Comparing symbols defs in %s with %s...\n", conf->kabi_dir_old,
	    conf->kabi_dir_new);

	walk_dir(conf->kabi_dir_old, false, check_symbol_file, conf);
}
